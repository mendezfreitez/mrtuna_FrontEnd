...
export default {
    ...
    methods:{
        ...mapMutations(['setListaMercados', 'setRepartidores']),
        traerRepartidores(){
            axios.get(`${this.url}traerRepartidores`, config).then(function(resp){
        ...
        eliminar(){
            axios.post(`${this.url}borrarRepartidor`, { 'id':this.idRepartidor }, config).then(function(resp){
            if(resp.status === 200){
                this.showNotif('top', resp.data.mensaje, resp.data.icono, resp.data.color, 'black')
